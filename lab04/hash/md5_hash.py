...
def md5(message):
    ...
    for i in range(0, len(message), 64):
        ...
        for j in range(64):
            ...
            # Khởi tạo các hằng số T, S và K
            # (Đoạn mã này thiếu các hằng số T và S, nhưng logic vòng lặp MD5 được thể hiện)
            # Giả sử các hằng số và phép toán left_rotate (a + f + K[j] + words[g], S[j]) được thực hiện ở đây

            temp = d
            d = c
            c = b
            a = b + left_rotate(a + 0xF5A82799 + words[g] & 0xFFFFFFFF, 3) 
            a = temp
